...
const QUICK_INGREDIENTS = ["Egg", "Cheese", "Rice", "Chicken", "Carrot", "Garlic", "Tomato", "Onion"].map((name) => ({
  name,
  ...
  return (
    ...
    <div className="relative overflow-hidden">
      ...
      <div className="relative min-h-screen">
        ...
        {/* Header */}
        <header className="flex items-center justify-center gap-4 py-6 px-4 sm:gap-12 sm:py-8">
          {" "}
          {/* Adjusted gap and added px for mobile */}
          <nav className="flex flex-wrap justify-center items-center gap-4 text-lg font-semibold text-white">
            {" "}
            {/* Added flex-wrap and justify-center for mobile */}
            <Link
              href="/"
        ...
        <main className="relative z-10 px-4 sm:px-6 py-8 sm:py-20">
          ...
          <div className="text-center mb-8 sm:mb-12 relative">
            ...
            <div className="relative">
              ...
              <div className="relative inline-block">
                <p className="text-base sm:text-2xl text-white font-extrabold max-w-2xl mx-auto bg-black/50 backdrop-blur-md px-6 py-3 rounded-full shadow-lg ring-2 ring-white/20 hover:scale-105 transition-all duration-300">
                  {" "}
          ...
          <div className="max-w-lg mx-auto mb-8 sm:mb-12 relative">
            <div className="relative group">
              <div
                className={`w-full min-h-16 px-4 py-3 border-4 border-white rounded-3xl shadow-2xl bg-white transform transition-all duration-300 ${
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {selectedIngredients.map((ingredient, index) => (
                    ...
                    >
                      {QUICK_INGREDIENTS.find((item) => item.name === ingredient)?.emoji || EMOJI_MAP[ingredient]}{" "}
                      {ingredient}
                      <button
          ...
          <div className="text-center mb-12 sm:mb-16">
            ...
            >
              <button className="group relative bg-gradient-to-r from-yellow-300 via-amber-400 to-amber-700 hover:from-yellow-300 via-amber-400 to-amber-700 text-white px-8 py-4 text-xl sm:text-2xl font-black rounded-full shadow-2xl hover:shadow-3xl transition-all duration-300 transform hover:scale-110 hover:-rotate-2 w-full sm:w-auto">
                {" "}
                {/* Original colors, adjusted padding for mobile */}
                ...
                {/* Button glow effect */}
                <div className="absolute inset-0 bg-gradient-to-r from-yellow-300 via-amber-400 to-amber-700 rounded-full blur-lg opacity-50 group-hover:opacity-75 transition-opacity duration-300"></div>{" "}
                {/* Original colors */}
              </button>
            </Link>
          </div>
      ...
      {showFilters && (
        <div className="fixed inset-x-4 top-24 sm:right-8 sm:left-auto sm:w-96 bg-white rounded-3xl shadow-2xl border-4 border-amber-300 p-6 sm:p-8 z-50 animate-slide-in">
          {" "}
          ...
          <div className="space-y-6 sm:space-y-8">
            ...
            <div>
              <label className="block font-bold text-gray-800 mb-3 text-base sm:text-lg">
                {" "}
                {/* Adjusted font size and margin for mobile */}⏰ Max cooking time:{" "}
                <span className="text-sky-600">{cookingDuration} min</span> {/* Original color */}
              </label>
              <input
                type="range"
                min="0"
            ...
            <div>
              <label className="block font-bold text-gray-800 mb-3 text-base sm:text-lg">
                {" "}
                {/* Adjusted font size and margin for mobile */}🥕 Max ingredients:{" "}
                <span className="text-sky-600">{ingredientCount}</span> {/* Original color */}
              </label>
              <input
                type="range"
                min="0"
      ...
      <section className="relative z-10 bg-white bg-opacity-90 py-12 sm:py-20 mt-12 sm:mt-16">
        {" "}
        {/* Adjusted padding and margin for mobile */}
        <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center">
          <h2 className="text-2xl sm:text-5xl font-black text-gray-800 mb-8 sm:mb-16">
            {" "}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-12">
            {" "}
            {/* Ensured single column on mobile */}
            {[
              {
                emoji: "🧺",
              ...
              >
                ...
                >
                  <div className="text-5xl sm:text-6xl mb-4 sm:mb-6 group-hover:animate-bounce">{step.emoji}</div>{" "}
                  {/* Adjusted font size and margin for mobile */}
                  ...
                  <p className="text-white font-semibold opacity-90 text-sm sm:text-base">{step.desc}</p>{" "}
